...
window.executeBatchDelete = async function() {
    ...
    window.loadEditorBookList = async function(page = 1) {
        ...
        try {
            ...
            const totalPages = Math.ceil(catalog.length / EDITOR_BOOKS_PER_PAGE) || 1;
            if (currentEditorPage > totalPages) currentEditorPage = totalPages;
            
            const startIndex = (currentEditorPage - 1) * EDITOR_BOOKS_PER_PAGE;
            const pageItems = catalog.slice(startIndex, startIndex + EDITOR_BOOKS_PER_PAGE);
